    ...
    <div className="space-y-2 py-2">
      {RECENT_FILES.slice(0, 3).map((file) => {
        return (
          <div
            key={file.id}
            className="group flex items-center justify-between art-border px-4 py-3 bg-surface-card hover:bg-bg-base transition-all cursor-pointer rounded-xl"
          >
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <h4 className="text-[10px] font-bold uppercase tracking-wider truncate">{file.name}</h4>
            </div>

            <ArrowRight
              size={10}
              className="text-text-main/20 group-hover:text-primary group-hover:translate-x-1 transition-all shrink-0"
            />
          </div>
        );
      })}
