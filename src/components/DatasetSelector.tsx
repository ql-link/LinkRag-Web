      ...
      <div
        ...
        className={cn(
          ...
          open
            ...
            : darkMode
              ? 'border-[#3a3a3a] bg-[#303030]/60 hover:border-[#4a4a4a]'
              : 'border-border-subtle bg-bg-base/30 hover:border-[#c9c9c9]',
        )}
      >
        {single && selectedSingleName ? (
          <span className={cn('truncate text-sm font-semibold', darkMode ? 'text-[#f2f2f2]' : 'text-text-main')}>
            {selectedSingleName}
          </span>
        ) : selectedKbInfo.length > 0 ? (
          <DatasetBadgeList items={selectedKbInfo} darkMode={darkMode} onRemove={removeDataset} />
        ) : (
          <span className={cn(single ? 'text-sm' : 'mono-label', darkMode ? 'text-muted' : 'text-text-main/40')}>
            {placeholder}
          </span>
        )}
        <ChevronDown
          ...
          className={cn(
            ...
            open && 'rotate-180',
            darkMode ? 'text-muted' : 'text-text-main/30',
          )}
        />
      </div>
      ...
      {open && (
        <div
          className={cn(
            ...
            darkMode ? 'border-[#3a3a3a] bg-[#303030]' : 'border-border-subtle bg-white',
          )}
        >
          {/* Search */}
          <div className="border-b border-border-subtle p-3">
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  'w-full rounded-lg py-2 pl-9 pr-4 text-xs focus:border-border-subtle focus:outline-none',
                  darkMode
                    ? 'bg-[#1f1f1f] border-[#3a3a3a] text-[#f2f2f2] placeholder:text-muted-soft'
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? darkMode
                        ? 'bg-[#1f1f1f] text-[#f2f2f2] border border-[#3a3a3a]'
                  ...
                  {isSelected && (
                    <div
                      className={cn(
                        ...
                        darkMode ? 'bg-[#f2f2f2]' : 'bg-text-main',
                      )}
                    >
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path
                          d="M2 5L4 7L8 3"
                          stroke={darkMode ? '#1f1f1f' : '#ffffff'}
                          strokeWidth="1.5"
                          strokeLinecap="round"
                        />
                      </svg>
