...
import { Activity, ArrowUpRight, BarChart3, CalendarDays, Clock3, Coins, RefreshCw, TriangleAlert } from 'lucide-react';
      ...
      <main className={cn(
        ...
        <section className="px-8 py-6 space-y-5">
          <div className="grid grid-cols-4 gap-3">
            <MetricCard darkMode={darkMode} label="总调用" value={summary?.totalCalls ?? 0} icon={<Activity size={16} />} />
            <MetricCard darkMode={darkMode} label="总 Token" value={summary?.totalTokens ?? 0} icon={<Coins size={16} />} />
            <MetricCard darkMode={darkMode} label="提示词 Token" value={summary?.promptTokens ?? 0} icon={<ArrowUpRight size={16} />} />
            <MetricCard darkMode={darkMode} label="平均延迟" value={`${summary?.averageLatencyMs ?? 0} ms`} icon={<Clock3 size={16} />} />
          </div>
          ...
          <div className={cn(
            ...
            <div className="grid gap-2 items-end h-[220px]" style={{ gridTemplateColumns: `repeat(${dailyUsage.length || 1}, minmax(0, 1fr))` }}>
              {dailyUsage.map((item) => (
                <div key={item.date} className="flex h-full flex-col items-center justify-end gap-2">
                  ...
                  <span className={cn("text-[10px] rotate-[-45deg] origin-center whitespace-nowrap", darkMode ? "text-[#858585]" : "text-text-main/45")}>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div className={cn(
              ...
              <div className="space-y-3">
                {dailyUsage.map((item) => (
                  <div key={item.date} className={cn("flex items-center justify-between rounded-lg px-3 py-2", darkMode ? "bg-[#1e1e1e]" : "bg-bg-base/60")}>
                    <div>
                      <p className={cn("text-sm font-bold", darkMode ? "text-[#e0e0e0]" : "text-text-main")}>{item.date}</p>
                      <p className={cn("mono-label", darkMode ? "text-[#858585]" : "text-text-main/40")}>调用 {formatNumber(item.calls)} 次</p>
                    </div>
                    <div className="text-right">
                      <p className={cn("text-sm font-bold", darkMode ? "text-[#e0e0e0]" : "text-text-main")}>{formatNumber(item.totalTokens)}</p>
            ...
            <div className={cn(
              ...
              <h3 className={cn("text-sm font-bold mb-4", darkMode ? "text-[#e0e0e0]" : "text-text-main")}>最近调用记录</h3>
              <div className="space-y-3">
                {logs.length === 0 ? (
                  <div className={cn("rounded-lg border border-dashed px-4 py-10 text-center", darkMode ? "border-[#3c3c3c] text-[#858585]" : "border-border-subtle text-text-main/40")}>
                    暂无用量记录
                  </div>
                ) : logs.map((log) => (
                  <div key={log.id} className={cn("rounded-lg px-3 py-3 border", darkMode ? "border-[#3c3c3c] bg-[#1e1e1e]" : "border-border-subtle bg-bg-base/55")}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className={cn("text-sm font-bold truncate", darkMode ? "text-[#e0e0e0]" : "text-text-main")}>{log.modelName}</p>
                        <p className={cn("mono-label mt-1", darkMode ? "text-[#858585]" : "text-text-main/40")}>
                          {log.providerType} · {formatTime(log.createdAt)}
                        </p>
                      </div>
                      <span className={cn("rounded-full px-2 py-1 text-[10px] font-bold", log.status === 'SUCCESS' ? (darkMode ? "bg-emerald-500/15 text-emerald-300" : "bg-emerald-50 text-emerald-700") : (darkMode ? "bg-red-500/15 text-red-300" : "bg-red-50 text-red-600"))}>
                        {log.status}
                      </span>
                    </div>
                    ...
                    {log.errorMessage && (
                      <div className={cn("mt-3 flex items-start gap-2 rounded-md px-3 py-2 text-[11px]", darkMode ? "bg-red-500/10 text-red-300" : "bg-red-50 text-red-600")}>
...
function StatPill({ darkMode, label, value }: { darkMode?: boolean; label: string; value: number }) {
  return (
    <div className={cn("rounded-md px-2 py-2", darkMode ? "bg-[#2d2d2d]" : "bg-white")}>
      ...
      <div className={cn("text-sm font-bold mt-1", darkMode ? "text-[#e0e0e0]" : "text-text-main")}>{formatNumber(value)}</div>
    </div>
  );
}
