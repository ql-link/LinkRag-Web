...
import { ArrowLeft, ArrowUpDown, ChevronDown, FileText, Loader2, Send, Upload } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Breadcrumb } from '@/components/Breadcrumb';
import { Routes } from '@/routes';
import { useToast } from '@/contexts/ToastContext';
import { useAuth } from '@/contexts/AuthContext';
import { getMessages, getConversations, sendMessage } from '@/services/chat';
...
function ParseAfterUploadSwitch({
  ...
  return (
    ...
    >
      ...
      <span className={cn('whitespace-nowrap leading-none', checked ? (darkMode ? 'text-[#3b82f6]' : 'text-primary') : (darkMode ? 'text-[#cccccc]' : 'text-text-main/70'))}>
        上传后立即解析
      </span>
    </button>
  );
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const matchedByConversation = conversation.lastConfigId
      && chatModels.some((config) => config.id === conversation.lastConfigId);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const message = await sendMessage(conversation.id, content, selectedModelConfigId ?? undefined);
      setMessages((prev) => [...prev, message]);
      if (selectedModelConfigId) {
        setConversation((prev) => (prev ? { ...prev, lastConfigId: selectedModelConfigId } : prev));
      }
      setInputValue('');
    } catch (error) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 px-0 py-5 min-h-0">
        <div className="h-full grid grid-cols-[250px_minmax(0,1fr)] gap-2">
          <aside
            className={cn(
          ...
          >
            <div className={cn('px-5 py-4 border-b', darkMode ? 'border-[#3c3c3c]' : 'border-border-subtle')}>
              <div className="flex items-center justify-between gap-2">
                <div>
                  <p className={cn('text-sm font-bold tracking-wide', darkMode ? 'text-[#e0e0e0]' : 'text-text-main')}>关联文件</p>
                </div>
                <button
                  type="button"
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4">
              {loadingFiles ? (
                ...
                <div className={cn('h-full flex items-center justify-center text-xs text-center px-2', darkMode ? 'text-[#858585]' : 'text-text-main/50')}>
                  当前知识库还没有文件
                </div>
              ) : (
                  ...
                  {sortedFiles.map((file) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <p className={cn('text-sm font-medium truncate', darkMode ? 'text-[#e0e0e0]' : 'text-text-main')}>{file.originalFilename}</p>
                      </div>
            ...
            <div className={cn('p-4 border-t', darkMode ? 'border-[#3c3c3c]' : 'border-border-subtle')}>
              <div
                ...
                className={cn(
                  ...
                  dragging
                    ? darkMode ? 'border-[#3b82f6] bg-[#3b82f6]/10' : 'border-primary bg-primary/5'
                    : darkMode ? 'border-[#3c3c3c] bg-[#1e1e1e]' : 'border-border-subtle bg-bg-base/40'
                )}
              >
                <div className="flex flex-col items-center justify-center gap-2 mb-3">
                  <Upload size={16} className={darkMode ? 'text-[#858585]' : 'text-text-main/45'} />
                  <p className={cn('text-xs text-center leading-tight', darkMode ? 'text-[#858585]' : 'text-text-main/60')}>
                    {uploading ? (
                      '上传中...'
                    ) : (
                      <>
                ...
                <input ref={fileInputRef} type="file" className="hidden" accept=".md,.markdown,.pdf,.docx,.txt" onChange={handleFileInputChange} />
              </div>
              <div className="mt-3 flex items-center justify-start gap-2">
                ...
                />
                <span className={cn('text-[8px] font-medium whitespace-nowrap leading-none', darkMode ? 'text-[#858585]' : 'text-text-main/45')}>
                  支持 md/pdf/docx/txt
                </span>
              </div>
            </div>
              ...
              {messages.length === 0 ? (
                <div className="h-full flex items-center justify-center">
                  <div className="text-center">
                    <p className={cn('text-2xl mb-3 font-sans font-medium', darkMode ? 'text-[#e0e0e0]' : 'text-text-main')}>
                      {displayName}，今天想聊点什么？
                    </p>
                ...
                >
                  ...
                  <div className="relative shrink-0">
                    <select
                      ...
                      onChange={(event) => setSelectedModelConfigId(event.target.value ? Number(event.target.value) : null)}
